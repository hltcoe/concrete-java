...
public class ConcreteKBEntityFactory {
  ...
  static {
    ...
    // Tool name arbitrary.
    tacKBMetadata = AnnotationMetadata.newBuilder().setConfidence(1.0f).setTool("KB Ground Truth").build();
  }
  ...
  /**
   * Example method for generating a Vertex whose type is a {@link Vertex.Kind.PERSON}.
   * 
   * @return
   */
  public static Vertex generatePersonVertex(String name, String nationality, float age) {
    // Generate a UUID.
    Concrete.UUID id = IdUtil.generateUUID();
    ...
    StringAttribute nameAttr = StringAttribute.newBuilder().setValue(name).setMetadata(tacKBMetadata).build();
    ...
    StringAttribute natlAttr = StringAttribute.newBuilder().setValue(nationality).setMetadata(tacKBMetadata).build();
    ...
    FloatAttribute ageAttr = FloatAttribute.newBuilder().setValue(age).setMetadata(tacKBMetadata).build();
