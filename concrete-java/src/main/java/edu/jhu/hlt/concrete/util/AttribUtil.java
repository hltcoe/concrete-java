...
public class AttribUtil {
  ...
  public static Message buildAttribute(FieldDescriptor field, Object value, Concrete.AnnotationMetadata metadata) throws ConcreteException {
    if (field == null)
      throw new ConcreteException("FieldDescriptor is NULL!  This probably means that you had a typo " + "in a call to FieldDescriptor.findFieldByName.");
    Descriptor attribDescriptor = field.getMessageType();
    if (attribDescriptor == MENTION_ATTRIBUTE) {
      return Graph.MentionAttribute.newBuilder().setMetadata(metadata).setValue((Concrete.EntityMentionRef) value).setUuid(IdUtil.generateUUID()).build();
    } else if (attribDescriptor == BOOLEAN_ATTRIBUTE) {
      return Graph.BooleanAttribute.newBuilder().setMetadata(metadata).setValue((Boolean) value).setUuid(IdUtil.generateUUID()).build();
    } else if (attribDescriptor == STRING_ATTRIBUTE) {
      return Graph.StringAttribute.newBuilder().setMetadata(metadata).setValue((String) value).setUuid(IdUtil.generateUUID()).build();
    } else if (attribDescriptor == FLOAT_ATTRIBUTE) {
      return Graph.FloatAttribute.newBuilder().setMetadata(metadata).setValue((Float) value).setUuid(IdUtil.generateUUID()).build();
    } else if (attribDescriptor == INT32_ATTRIBUTE) {
      return Graph.Int32Attribute.newBuilder().setMetadata(metadata).setValue((Integer) value).setUuid(IdUtil.generateUUID()).build();
      ...
      return Graph.CommunicationGUIDAttribute.newBuilder().setMetadata(metadata).setValue((Concrete.CommunicationGUID) value).setUuid(IdUtil.generateUUID())
          .build();
    } else if (attribDescriptor == STRING_FLOAT_MAP_ATTRIBUTE) {
      return Graph.StringFloatMapAttribute.newBuilder().setMetadata(metadata).addAllValue(buildStringFloatMap(value)).setUuid(IdUtil.generateUUID()).build();
    } else if (attribDescriptor == VERTEX_KIND_ATTRIBUTE) {
      return Graph.VertexKindAttribute.newBuilder().setMetadata(metadata).setValue((Graph.Vertex.Kind) value).setUuid(IdUtil.generateUUID()).build();
    } else {
  ...
  private static List<Graph.StringFloatMapAttribute.Entry> buildStringFloatMap(Object object) throws ConcreteException {
    try {
      ...
      for (Map.Entry<Object, Object> entry : map.entrySet()) {
        result.add(Graph.StringFloatMapAttribute.Entry.newBuilder().setKey((String) entry.getKey()).setValue((Float) entry.getValue()).build());
      }
      return result;
