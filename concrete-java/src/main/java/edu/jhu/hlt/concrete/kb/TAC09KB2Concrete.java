...
/*
 * 
 * # Required* / Desired Concrete Fields # Vertex # *uuid # kind* (PERSON, ORGANIZATION, GPE, UNKNOWN) # name* [StringAttribute: uuid, AttributeMetadata
 * metadata, value] # Communication # *guid # *uuid # text # (language_id) # title # kind (WIKIPEDIA) # *knowledge_graph # KnowledgeGraph (EMPTY) # *uuid #
 * *vertex
 * 
 * # Mappings: # TACKB Concrete # ===== ======== # wiki_text Communication->text # entity Vertex # entity->wiki_title Communication->title ?? # entity->type
 * Vertex->kind # entity->id hashed or mapped to Vertex->uuid # entity->id Communication->guid (TAC09KB_E00392) # entity->name Vertex->name (possibly
 * normalized) # fact:fullname Vertex->name # fact:??? RawKeyValuePair # fact:name RawKeyValuePair->key # fact->{TEXT} RawKeyValuePair->value
 * 
 * Vertex UUID uuid VertexKindAttribute kind => {PERSON, ORGANIZATION, GPE, UNKNOWN} StringAttribute name* UUID uuid AttributeMetadata metadata string tool
 * double timestamp float confidence string value StringAttribute comment* CommunicationGUIDAttribute communication_guid* UUID uuid AttributeMetadata metadata
 * CommunicationGUID value
 * 
 * Communication CommunicationGUID guid string corpus_name (TAC09KB) string communication_id (E000008) UUID uuid string text LanguageIdentification language_id
 * UUID uuid AnnotationMetadata metadata? LanguageProb language* string language (ISO 639-3) float probability
 */
...
public class TAC09KB2Concrete {
  ...
  static {
    annotation_metadata = AnnotationMetadata.newBuilder().setConfidence(1.0f).setTool("TAC09KB2Concrete.java").setTimestamp(timestamp).build();
    language_prob = LanguageProb.newBuilder().setLanguage("eng").setProbability(1.0f).build();
    language_id = LanguageIdentification.newBuilder().addLanguage(language_prob).setUuid(IdUtil.generateUUID()).setMetadata(annotation_metadata).build();
  }
  ...
  /**
   * Generate the unique {@link UUID} for a given TAC KBID. (This perhaps belongs in IdUtil.java)
   * 
   * @param tac_id
   *          the TAC09 KBID for the entity (such as E000042)
   * @return a UUID guaranteed to be the same across different calls to this routine
   */
  ...
  class KBHandler extends DefaultHandler {
    ...
    public void endElement(String uri, String localName, String qualifiedName) {
      if (qualifiedName.equals("wiki_text")) {
        ...
        Communication communication = Communication.newBuilder().setUuid(uuid).setGuid(guid).setText(commText).addLanguageId(language_id)
            .setKind(Communication.Kind.WIKIPEDIA).build();
        ...
        vb.addKind(VertexKindAttribute.newBuilder().setValue(this.currentEntity.getKind()).setMetadata(annotation_metadata).setUuid(IdUtil.generateUUID()));
        ...
        for (String key : this.otherAttributes.keySet())
          vb.addOtherAttributes(Graph.LabeledAttribute.newBuilder().setUuid(IdUtil.generateUUID()).setMetadata(annotation_metadata).setLabel(key)
              .setValue(this.otherAttributes.get(key)).build());
