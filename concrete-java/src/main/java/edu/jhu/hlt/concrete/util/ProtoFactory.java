...
/**
 * Utility class for easily generating various protocol buffer objects in the {@link Concrete} class.
 * 
 * @author max
 * 
 */
public class ProtoFactory {
   ...
   *          - name of the corpus
   * @param commId
   *          - id of the {@link Communication}
   * @return a {@link CommunicationGUID} object
   */
  public static CommunicationGUID generateCommGuid(String corpusName, String commId) {
    return CommunicationGUID.newBuilder().setCorpusName(corpusName).setCommunicationId(commId).build();
  }
   ...
   * 
   * @param guid
   *          - the {@link CommunicationGUID} object to use for this {@link Communication}
   * @param graph
   *          - the {@link KnowledgeGraph} object to use for this {@link Communication}
   * @return a {@link Communication} object
   */
  public static Communication generateCommunication(CommunicationGUID guid) {
    return Communication.newBuilder().setGuid(guid).setUuid(IdUtil.generateUUID()).build();
  }

  public static void writeCommunication(Communication c, Path outputPath) throws ConcreteException {
    try {
  ...
  public static Communication readCommunicationFromPath(Path pathToComm) throws ConcreteException {
    try {
