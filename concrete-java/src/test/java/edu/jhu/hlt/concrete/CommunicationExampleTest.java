...
public class CommunicationExampleTest {
  ...
  public void setUp() throws Exception {
    ...
    Communication.Builder cb = mockComm.toBuilder().setText(docText);
    this.comm = cb.build();
  }
      ...
      for (Section sec : secList) {
        ...
        for (SentenceSegmentation sentSeg : sentSegList) {
          ...
          for (Sentence sent : sentList) {
            ...
            List<Tokenization> tokenizationList = sent.getTokenizationList();
            ...
            for (Tokenization tokenization : tokenizationList) {
              ...
              List<TokenTagging> posTags = tokenization.getPosTagsList();
              for (TokenTagging tt : posTags) {
                List<TaggedToken> tagTokenList = tt.getTaggedTokenList();
